...
export default function EnterpriseAddonSuccess() {
  ...
  useEffect(() => {
    const verifyAddonPurchase = async () => {
      try {
        ...
        console.log('🔍 Verifying add-on session:', sessionId);
        ...
        console.log('✅ Add-on credits added:', data.credits);

        setStatus('success');
        setCreditsAdded(data.credits);
        setMessage(data.message || `${data.credits} credits have been added to your account!`);
        ...
        setStatus('error');
        setMessage(error instanceof Error ? error.message : 'Failed to verify purchase');
      }
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1a1a1a] via-[#2d2d2d] to-[#1a1a1a] flex items-center justify-center p-4">
      ...
      >
          ...
          {status === 'success' && (
            ...
            >
              <div className="relative">
                ...
                <Plus className="w-8 h-8 text-[#ffd700] absolute -top-2 -right-2" />
              </div>
            </motion.div>
          )}
          {status === 'error' && (
        ...
        <h1 className="text-2xl font-bold text-center mb-4">
          ...
          {status === 'success' && (
            <span className="text-green-500">Credits Added!</span>
          )}
          {status === 'error' && (
        ...
        {status === 'success' && creditsAdded > 0 && (
          <motion.div
            ...
            className="bg-[#1a1a1a]/50 rounded-xl p-6 text-center"
          >
