...
import type { ShotType, CameraMovement, LightingSetup } from "../types-extended";

export const SHOT_TYPES: Record<ShotType, string> = {
...
export const LIGHTING_SETUPS: Record<LightingSetup, string> = {
...
export const CINEMATOGRAPHY_PROMPT_TEMPLATES = {
  establishing: (location: string, mood: string) => 
    `Cinematic establishing shot of ${location}, ${mood} atmosphere, golden hour lighting, 35mm anamorphic lens, shallow depth of field, film grain, professional color grading`,
  
  character: (description: string, emotion: string) => 
    `Portrait shot of ${description}, showing ${emotion} emotion, Rembrandt lighting, 85mm lens, bokeh background, cinematic depth, professional film quality`,
  
  action: (action: string, intensity: string) => 
    `Dynamic ${intensity} action shot, ${action}, handheld camera movement, high shutter speed, motion blur, gritty realistic style, 24mm wide lens`,
  
  product: (product: string, style: string) => 
    `Professional product photography of ${product}, ${style} aesthetic, studio lighting setup, macro detail, reflections, ultra sharp focus, commercial quality`,
  
  atmosphere: (setting: string, elements: string[]) => 
    `Atmospheric cinematic shot, ${setting}, featuring ${elements.join(', ')}, volumetric lighting, ray tracing, photorealistic render, 8K quality`
};
