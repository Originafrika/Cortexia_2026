...
export default async function handler(req: any, res: any) {
  if (req.method === 'GET') {
    try {
      ...
      // 3. Determine Tier
      const isEnterprise = user.type === 'enterprise_admin' || user.type === 'enterprise_member' || user.type === 'enterprise';
      ...
      return res.status(500).json({ error: 'Failed to fetch access data', details: error.message });
    }
  } else {
